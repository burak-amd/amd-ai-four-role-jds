#!/bin/bash
# Deploy amd-four-jds-share to GitHub Pages (burak-amd account).
set -euo pipefail

DECK="$(cd "$(dirname "$0")" && pwd)"
REPO_NAME="amd-ai-four-role-jds"
...
if [ -z "$(git status --porcelain 2>/dev/null)" ] && [ -n "$(git rev-parse HEAD 2>/dev/null)" ]; then
  ...
  git add -A
  git diff --cached --quiet || git commit -m "Update AMD AI four role job descriptions"
fi
